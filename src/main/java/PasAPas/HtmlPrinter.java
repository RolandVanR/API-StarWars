...
//                    }
	...
	public String printHtmlDetailsFilms(JSONObject filmData) {
        if (filmData != null) {
            ...
            // Vérifie si la clé "results" est présente dans le JSON
            if (filmData.has("result")) {
                JSONArray results = filmData.getJSONArray("result");

                // Parcourt chaque objet planète
                for (int i = 0; i < results.length(); i++) {
                    JSONObject film = results.getJSONObject(i);
                    htmlBuilder.append("<h2>").append("Title: ").append(film.getJSONObject("properties").getString("title")).append("</h2>");
                    htmlBuilder.append("<p>").append("Episode n°: ").append(film.getJSONObject("properties").getInt("episode_id")).append("</p>");
                    htmlBuilder.append("<p>").append("Opening: ").append(film.getJSONObject("properties").getString("opening_crawl")).append("</p>");
                    htmlBuilder.append("<p>").append("Réalisateur: ").append(film.getJSONObject("properties").getString("director")).append("</p>");
                    htmlBuilder.append("<p>").append("Producteurs: ").append(film.getJSONObject("properties").getString("producer")).append("</p>");
                    htmlBuilder.append("<p>").append("Date de sortie: ").append(film.getJSONObject("properties").getString("release_date")).append("</p>");
                    System.out.println(codeHtml);
               }
            }
            htmlBuilder.append("</body></html>");
